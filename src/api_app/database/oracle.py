...
def crear_usuarios_por_defecto():
    ...
    logger.info("=== INICIANDO CREACIÓN DE USUARIOS POR DEFECTO ===")
    ...
    try:
        with get_oracle_connection() as conn:
            cursor = conn.cursor()
            logger.info("Conexión a Oracle establecida para crear usuarios")
            
            for usuario in usuarios_defecto:
                try:
                    logger.info(f"Procesando usuario: {usuario['username']}")
                    ...
                    logger.info(f"Usuario {usuario['username']} existe: {existe > 0}")
                    
                    if existe == 0:
                        # Hashear la contraseña
                        logger.info(f"Hasheando contraseña para {usuario['username']}")
                        password_hash = pwd_context.hash(usuario["password"])
                        
                        # Insertar el usuario
                        logger.info(f"Insertando usuario {usuario['username']} en BD")
                        cursor.execute(
                            "INSERT INTO USERS (USERNAME, PASSWORD_HASH, ROLE) VALUES (:1, :2, :3)",
                            [usuario["username"], password_hash, usuario["role"]]
                        )
                        logger.info(f"✓ Usuario por defecto creado: {usuario['username']} ({usuario['role']})")
                    else:
                        logger.info(f"✓ Usuario por defecto ya existe: {usuario['username']}")
                
                except oracledb.DatabaseError as e:
                    logger.error(f"✗ Error de BD creando usuario {usuario['username']}: {e}")
                    raise
                except Exception as e:
                    logger.error(f"✗ Error general creando usuario {usuario['username']}: {e}")
                    raise
            
            conn.commit()
            logger.info("Commit realizado exitosamente")
            logger.info("=== CREACIÓN DE USUARIOS FINALIZADA ===")
    
    except Exception as e:
        logger.error(f"✗✗✗ Error crítico en crear_usuarios_por_defecto: {e}")
        raise
...
def crear_tablas_oracle():
    ...
    with get_oracle_connection() as conn:
        ...
        for script in sql_scripts:
            try:
                ...
                error, = e.args
                # 955: objeto ya existe
                # 1430: columna ya existe
                # 1408: índice ya existe en esa columna
                if error.code in (955, 1430, 1408):
                    logger.info("Tabla, índice o columna ya existe, continuando...")
                else:
    ...
    # Crear usuarios por defecto después de crear las tablas
    logger.info("Procediendo a crear usuarios por defecto...")
    try:
