...
def registrar_lote_procesamiento(
    archivo_entrada, total_registros, total_anomalias, tasa_anomalias, archivo_salida, estado="COMPLETADO"
):
...
async def estadisticas_alertas(
    ...
    try:
        with get_oracle_connection() as conn:
            ...
            stats_generales = dict(
                zip(
                    ["total_alertas", "paises_afectados", "lineas_afectadas", "score_promedio", "score_maximo"],
                    cursor.fetchone(),
                )
            )
            ...
            distribucion_tipos = [{"tipo": row[0], "cantidad": row[1]} for row in cursor.fetchall()]
            ...
            top_paises = [{"codigo_pais": row[0], "cantidad": row[1]} for row in cursor.fetchall()]
            ...
            tendencia_diaria = [{"fecha": row[0], "cantidad": row[1]} for row in cursor.fetchall()]
...
async def aprendizaje_incremental(file: UploadFile = File(...), background_tasks: BackgroundTasks = BackgroundTasks()):
    """
...
def procesar_aprendizaje_incremental(archivo_path: str, df: pd.DataFrame):
    ...
    estado_inicial = {
        ...
        "categorias": {pais: stats.get("CATEGORIA", "Desconocido") for pais, stats in stats_dict.items()},
    }
    ...
    try:
        ...
        if oracle_pool:
            id_aprendizaje = registrar_inicio_aprendizaje(archivo_path, len(df), estado_inicial)
        ...
        estadisticas_globales = {"llamadas_total": 0, "minutos_total": 0, "destinos_total": 0, "registros_total": 0}
                ...
                if pais in stats_dict:
                    ...
                    for campo in ["LLAMADAS_MEAN", "MINUTOS_MEAN", "DESTINOS_MEAN"]:
                        if campo in stats_dict[pais]:
                            stats_dict[pais][campo] = stats_dict[pais][campo] * 0.7 + stats_nuevas[campo] * 0.3
                    ...
                    if categoria_anterior and categoria_anterior != nueva_categoria:
                        cambios_categoria[pais] = {"anterior": categoria_anterior, "nueva": nueva_categoria}
                else:
                    # País nuevo
...
async def recargar_modelo():
    ...
    try:
        if cargar_modelo():
            return {"mensaje": "Modelo recargado exitosamente", "timestamp": datetime.now()}
        else:
...
async def resumen_diario(fecha: str = Query(..., description="Fecha en formato YYYY-MM-DD")):
    ...
    try:
        ...
        archivos_dia = [
            f for f in os.listdir(OUTPUT_PATH) if f.startswith("resultados_") and fecha.replace("-", "") in f
        ]
        ...
        for tipo, cnt in anomalias["tipo_anomalia"].value_counts().items():
            distribucion_tipos[tipo] = {"cantidad": int(cnt), "porcentaje": round(cnt / len(anomalias) * 100, 2)}
...
async def obtener_tendencias(dias: int = Query(7, description="Número de días a analizar")):
    ...
    try:
        ...
        for i in range(dias):
            ...
            archivos = [
                f for f in os.listdir(OUTPUT_PATH) if f.startswith("resultados_") and fecha_i.strftime("%Y%m%d") in f
            ]
            ...
            tendencias.append(
                {"fecha": fecha_i.strftime("%Y-%m-%d"), "registros": registros, "anomalias": anom_count, "tasa": tasas}
            )
...
async def exportar_anomalias(
    ...
    try:
        ...
        return FileResponse(out_path, media_type="text/csv", filename=f"anomalias_{fecha_inicio}_{fecha_fin}.csv")
...
async def obtener_logs_recientes(limite: int = Query(100, description="Número de logs a retornar")):
    ...
    try:
        ...
        # Ordenar y recortar
        logs_ordenados = sorted(logs, key=lambda x: x.get("timestamp", ""), reverse=True)[:limite]

        return {"total_logs": len(logs_ordenados), "logs": logs_ordenados}
...
async def listar_paises():
    ...
    return {
        ...
        "paises": sorted(paises, key=lambda x: x["registros_historicos"], reverse=True),
    }
...
async def estadisticas_pais(codigo_pais: str):
    ...
    if codigo_pais not in stats_dict:
        raise HTTPException(status_code=404, detail=f"País {codigo_pais} no encontrado")
...
async def historial_aprendizaje(limite: int = Query(30, description="Número de registros a mostrar")):
    """
    ...
    try:
        with get_oracle_connection() as conn:
            ...
            return {"total": len(historiales), "historiales": historiales}
    ...
    try:
        with get_oracle_connection() as conn:
            ...
            if detalle["DETALLE_CAMBIOS_CATEGORIA"]:
                try:
                    detalle["DETALLE_CAMBIOS_CATEGORIA"] = json.loads(detalle["DETALLE_CAMBIOS_CATEGORIA"].read())
                except:
...
async def estadisticas_evolucion():
    ...
    try:
        with get_oracle_connection() as conn:
            ...
            evolucion_paises = [
                {"fecha": row[0], "paises_antes": row[1], "paises_despues": row[2], "paises_nuevos": row[3]}
                for row in cursor.fetchall()
            ]
